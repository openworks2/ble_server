...
const bleserver = {
  ...
  receive(receiveData, scanner) {
    ...
    for (let idx in receiveData) {
      ...
      if (
        _this.beaconData.hasOwnProperty(mac) &&
        Object.keys(_this.scannerGroup).includes(scanner)
      ) {
        // Unknown= 배터리/3축정보
        if (type !== "Unknown") {
  ...
  unKnownHandler(data) {
    ...
    let bettery_dec = converter.hexToDec(`0x${bettery}`);
    bettery_dec = 99 - (100 - bettery_dec) * 1.65;
    bettery_dec = bettery_dec < 0 ? 0 : Math.ceil(bettery_dec);
          ...
          if (err) {
            ...
            if (!_this.beaconData[mac].ble_input_time) {
              const logData = {
                ...
                co_index:
                  data.bc_used_type === 1 ? data.wk_co_index : data.vh_co_index,
                co_name:
                  data.bc_used_type === 1 ? data.wk_co_name : data.vh_co_name,
                nation: data.bc_used_type === 1 ? data.wk_nation : null,
                position: data.bc_used_type === 1 ? data.wk_position : null,
                number:
                  data.bc_used_type === 1 ? data.wk_phone : data.vh_number,
              };
  ...
  outBeacon(data) {
    ...
    const { mac, battery, battery_timestamp } = data;
    const _updateData = {
      bc_input_time: null,
